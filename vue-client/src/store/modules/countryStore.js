...
const getters = {
    getOrderCountries (state) {
        return state.loadedCountries
    },
    countryExists (state) {
        return state.countryExists
    }
}
...
const actions = {
    loadCountries ({ commit }) {
        ...
        firestore.collection('countries').orderBy('country', 'asc').onSnapshot(querySnapshot => {
          const files = []  
          ...
          commit('setCountries', files)
          commit('setLoading', false)
        })
      },
      addCountry ({commit, state}, payload) {
        commit('setLoading', true)
        const countryS = payload.country.split(" ");
        for (let i = 0; i < countryS.length; i++) {
          var j = countryS[i].charAt(0).toUpperCase();
          countryS[i] = j + countryS[i].substr(1).toLowerCase();
        }
        const capitalizeCountry = countryS.join(" ");
        const country = {
        ...
        firestore.collection('countries').where('country', '==', capitalizeCountry)
        .get().then(querySnapshot => {
          if (querySnapshot.size > 0) {
            commit('setCountryExists', true)
            commit('setLoading', false)
          } else {
            firestore.collection('countries').add(country).then(docRef => {
              country.id = docRef.id
              firestore.collection('countries').doc(docRef.id).update({
                id: docRef.id
              })
              if (payload.logo) {
                const file = payload.logo
                const url = docRef + '.png'
                countriesImagesRef.child(url).put(file).then(snapshot => {
                  country.picture = snapshot.downloadURL
                  firestore.collection('countries').doc(docRef.id).update({
              ...
              if (!payload.checked) {
                ...
                users.forEach(doc => {
                  if (doc.data().role === 'ADMIN' || doc.data().role === 'MASTER_ADMIN' ) {
                    db.ref(doc.id + '/' + docRef.id).set({
                      title: 'Comprobar país añadido',
                      content: payload.country + '. Creado por ' + user.data().name + ' ' + user.data().surnames
                    })
                  }
                })
              }
              commit('addCountry', country)
              commit('setCountryExists', false)
              commit('setLoading', false)
          })
        }
      })
...
const mutations = {
    ...
    setCountryExists (state, payload) {
        state.countryExists = payload
    },
    addCountry (state, payload) {
        state.loadedCountries.push(payload)
    },
    setCountryPDF (state, payload) {
      state.countryPDF = payload
    }
}
