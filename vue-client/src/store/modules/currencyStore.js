...
const getters = {
    getOrderCurrencies (state) {
        return state.loadedCurrencies
    },
    currencyExists (state) {
        return state.currencyExists
    }
}
...
const actions = {
    loadCurrencies ({ commit }) {
        ...
        firestore.collection('currencies').orderBy('currency', 'asc').onSnapshot(querySnapshot => {
          const files = []  
          ...
          commit('setCurrencies', files)
          commit('setLoading', false)
        })
      },
      addCurrency ({commit, state}, payload) {
        commit('setLoading', true)
        const currencyS = payload.currency.split(" ");
        for (let i = 0; i < currencyS.length; i++) {
          var j = currencyS[i].charAt(0).toUpperCase();
          currencyS[i] = j + currencyS[i].substr(1).toLowerCase();
        }
        const capitalizeCurrency = currencyS.join(" ");
        const currency = {
          currency      : capitalizeCurrency,
        ...
        firestore.collection('currencies').where('currency', '==', capitalizeCurrency)
        .get().then(querySnapshot => {
          if (querySnapshot.size > 0) {
            commit('setCurrencyExists', true)
            commit('setLoading', false)
          } else {
            firestore.collection('currencies').add(currency).then(docRef => {
              currency.id = docRef.id
              firestore.collection('currencies').doc(docRef.id).update({
                id: docRef.id
              })
              if (!payload.checked) {
                ...
                db.ref('notifications/' + docRef.id).set({
                  name: payload.currency,
                  user: user.data().name + ' ' + user.data().surnames,
                  type: 'create',
                  site: 'currencies'
                })
              }
              commit('addCurrency', currency)
              commit('setCurrencyExists', false)
              commit('setLoading', false)
          })
        }
      })
    ...
    //   }
    // },
    checkCurrency({commit}, payload) {
      commit('setLoading', true)
      if (!payload.checked) {
        firestore.collection('currencies').doc(payload.id).update({
          checked: true
        }).then(() => {
          db.ref('notifications/' + payload.id).remove().then(() => {
            commit('setLoading', false)
          })
        })
      } 
    },
    softDeleteCurrency({commit}, payload) {
      commit('setLoading', true)
      if (!payload.deleted) {
        firestore.collection('currencies').doc(payload.id).update({
          deleted: true
    ...
    deleteCurrency({commit}, payload) {
      commit('setLoading', true)
      if (!payload.checked) {
        firestore.collection('currencies').doc(payload.id).delete().then(() => {
          db.ref('notifications/' + payload.id).remove().then(() => {
            commit('setLoading', false)
          })
        })
...
const mutations = {
    setCurrencies (state, payload) {
        state.loadedCurrencies = payload
    },
    setCurrencyExists (state, payload) {
        state.currencyExists = payload
    },
    addCurrency (state, payload) {
        state.loadedCurrencies.push(payload)
    },
    setCurrencyPDF (state, payload) {
      state.currencyPDF = payload
    }
}
