...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            updatedBrand: function (brand, web) {
                let checked = false
                if (this.user.role === 'ADMIN') {
                    checked = true
                }
                this.$store.dispatch('updatedBrand', {
                    id          : this.isEdit.id,
                    brand       : brand,
                    web         : web,
                    checked     : checked,
                    updatedFor  : this.userId 
                })
                this.isEdit.id = ''
                this.isEdit.state = false
            },
            deletedBrand(id, deleted) {
                const brand = {
                    id: id,
                    deleted: deleted
                }
                if (!deleted) {
                    this.$store.dispatch('deletedBrand', brand)
                }
            },
            checkBrand: function (file) {
                if (!file.checked) {
                    this.$store.dispatch('checkBrand', file)
                }
            },
            reset: function () {
                this.brand      = '',
                this.web        = '',
                this.logo       = null
                this.$store.dispatch('clearBrandExists')
            },
            isEditBrand: function (id) {
                this.isEdit.state = true
                this.isEdit.id = id
            },
            exportDataPDF: function () {
                this.$store.dispatch('exportDataPDF')
            }
        }
    }
</script>
